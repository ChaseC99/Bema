  ...
  try {
    if (request.decodedToken) {
      ...
      if (searchQuery.includes("kaid_")) {
        return db.query("SELECT STRING_AGG(DISTINCT entry_author, ', ') as contestant_names, entry_author_kaid as contestant_kaid FROM entry WHERE entry_author_kaid LIKE $1 GROUP BY entry_author_kaid", ["%" + searchQuery + "%"], res => {
          if (res.error) {
        ...
        return db.query("SELECT STRING_AGG(DISTINCT entry_author, ', ') as contestant_names, entry_author_kaid as contestant_kaid FROM entry WHERE entry_author LIKE $1 GROUP BY entry_author_kaid", ["%" + searchQuery + "%"], res => {
          if (res.error) {
            return handleNext(next, 500, "There was a problem searching for this contestant.", res.error);
          }
          response.json({
